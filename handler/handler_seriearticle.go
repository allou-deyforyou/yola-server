...
func (h *Handler) SerieArticle(w http.ResponseWriter, r *http.Request) {
	...
	source, _ := source.ParseSerieSource(movieSource.Name, movieSource)

	response := source.SerieArticle(ctx, link)
